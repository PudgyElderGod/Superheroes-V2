...
def test_ability_instance():
    ...
    big_strength = superheroes.Ability("Apocalypse Rail", 300)
    assert big_strength
...
def test_ability_name():
    ...
    big_strength = superheroes.Ability("Apocalypse Rail", 300)
    assert big_strength.name == "Apocalypse Rail"


def test_ability_attack():
    # Test for correct attack value
    test_runs = 400
    big_strength = superheroes.Ability("Apocalypse Rail", 400)
...
def test_weapon_instance(fluid_attack = superheroes.Weapon("Ferrofluidic Assault", 200)
    assert "Weapon" in str(fluid_attack)


def test_weapon_attack():
    fluid_attack = superheroes.Weapon("Ferrofluidic Assault", 200)
    test_runs = 100
    for _ in range(0, test_runs):
        attack = fluid_attack.attack()
        assert attack <= 200 and attack >= 100


# Test Heroes Class
def test_hero_instance():
    Atlas = superheroes.Hero("Atlas")
    assert Atlas
...
def test_hero_add_ability():
    big_strength = superheroes.Ability("Apocalypse Rail", 300)
    Atlas = superheroes.Hero("Atlas")
    assert len(Atlas.abilities) == 0
    Atlas.add_ability(big_strength)
    assert len(Atlas.abilities) == 1
    # Check for correct type
    assert "Ability" in str(Athena.abilities[0])
    assert Athena.abilities[0].name == "Apocalypse Rail"
...
def test_hero_add_multi_ability():
    ...
    Atlas.add_ability(big_strength)
    assert len(Atlas.abilities) == 1
    Atlas.add_ability(speed)
    assert len(Atlas.abilities) == 2
    # Check for correct type
    assert "Ability" in str(Atlas.abilities[0])
    assert Atlas.abilities[0].name == "Apocalypse Rail"


def test_hero_attack_ability():
    big_strength = superheroes.Ability("Apocalypse Rail", 30000)
    atlas = superheroes.Hero("Atlas")
    assert atlas.attack() == 0
    atlas.add_ability(big_strength)
    attack = atlas.attack()
...
def test_hero_ability_attack_mean_value():
    ...
    big_strength = superheroes.Ability("Apocalypse Rail", strength)
    atlas.add_ability(big_strength)
...
def test_hero_attack_weapon():
    big_strength = superheroes.Ability("Apocalypse Rail", 200)
    Atlas = superheroes.Hero("Atlas")
    Atlas.add_ability(big_strength)
    test_runs = 100
    for _ in range(0, test_runs):
...
def test_hero_multi_weapon_attack():
    strength = superheroes.Weapon("Apocalypse Rail", 200)
    terrashima = superheroes.Weapon("Terrashima", 700)
    Atlas = superheroes.Hero("Atlas")
    Atlas.add_ability(strength)
    Atlas.add_ability(terrashima)
    assert len(Atlas.abilities) == 2
...
def test_hero_weapon_ability_attack():
    ...
    Terrashima = superheroes.Weapon("Terrashima", 700)
    Atlas = superheroes.Hero("Atlas")
    Atlas.add_ability(quickness)
    Atlas.add_ability(terrashima)
    assert len(Atlas.abilities) == 2
    attack_avg(Atlas, 0, 2000)
...
def test_team_hero():
    team = superheroes.Team("One")
    jodie = superheroes.Hero("Jodie")
    team.add_hero(jodie)
    assert len(team.heroes) == 1
    assert team.heroes[0].name == "Jodie Holmes"


def test_team_remove_hero():
    team = superheroes.Team("One")
    jodie = superheroes.Hero("Jodie Holmes")
    team.add_hero(jodie)
    assert team.heroes[0].name == "Jodie Holmes"
    team.remove_hero("Jodie Holmes")
...
def test_team_remove_unlisted():
    ...
    jodie = superheroes.Hero("Jodie Holmes")
    team.add_hero(jodie)
    code = team.remove_hero("Atlas")
    assert code == 0


def test_team_remove_empty_list():
    team = superheroes.Team("One")
    assert team.remove_hero("Atlas") == 0
...
def test_print_heroes():
    ...
    assert "Jodie Holmes" in output_string
    assert "Atlas" in output_string
